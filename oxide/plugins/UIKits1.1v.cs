...
namespace Oxide.Plugins
{
    ...
    public class UIKits : RustPlugin
    {
        ...
        public class KitList
        {
            ...
            [JsonProperty(TranslationRU ? "Сообщение при отсутствии доступа"       : "No Permission Message")]
            public string NoPermissionMessage = "Приобрести: phxrust.gamestores.app";

            [JsonProperty(TranslationRU ? "Лимит использований (0 - без ограничений)" : "Usage Limit")]
        ...
        {
            // Возвращаем все категории, которые не скрыты
            return _KitList.Where(k => !k.Hide).ToList() ?? new List<CategoryList>();
        }
        ...
        private List<ItemList> GetItemPlayer(BasePlayer player)
        {
            ...
            {
                foreach (Item item in player.inventory.containerWear.itemList.Where(item => item != null && item.info != null))
                {
            ...
            {
                foreach (Item item in player.inventory.containerMain.itemList.Where(item => item != null && item.info != null))
                {
            ...
            {
                foreach (Item item in player.inventory.containerBelt.itemList.Where(item => item != null && item.info != null))
                {
        ...
        void KitFunctional(BasePlayer player, string command, string[] args)
        {
            ...
            switch (param)
            {
                ...
                case "kremove":
                {
                    ...
                    var findKit = find.KitList.FirstOrDefault(p => p.Name.Equals(paramKit, StringComparison.OrdinalIgnoreCase));
                    if (findKit == null)
                    {
            ...
            if (args.HasArgs())
            {
                switch (args.Args[0])
                {
                    case "viewKits":
                    {
                        ...
                        var findKit = findElement.KitList.FirstOrDefault(p => p.Name.Equals(args.Args[2], StringComparison.OrdinalIgnoreCase));
                    ...
                    case "giveKit":
                    {
                        ...
                        if (!permission.UserHasPermission(player.UserIDString, findKit.Permission))
                        {
                            player.ChatMessage("У вас нет доступа к этому киту. Купите VIP на phxrust.gamestores.app");
                            return;
                        }

                        var findCooldown = _playerData[player.userID].FirstOrDefault(p => p.DisplayName.Equals(findKit.Name, StringComparison.OrdinalIgnoreCase) && p.Number == findElement.Number);

                        if (findKit.UsageLimit > 0 && findCooldown != null && findCooldown.UsageCount >= findKit.UsageLimit)
                        {
                            player.ChatMessage(GetLang("KitUsageLimitReached", player.UserIDString));
                            return;
                        }
                        ...
                        if (findCooldown != null)
                        {
                            if (findCooldown.Cooldown - Facepunch.Math.Epoch.Current > 0)
                            {
                                ...
                                player.ShowToast(1, Cooldown);
                                return;
                            }

                            findCooldown.UsageCount++;
                        }
                        else
                        {
            ...
            lang.RegisterMessages(new Dictionary<string, string>
                {
                    ...
                    ["UI_Minutes"]            = "{0}м",
                    ["UI_Seconds"]            = "{0}c",
                    ["KitUsageLimitReached"]  = "ВЫ ДОСТИГЛИ ЛИМИТА ИСПОЛЬЗОВАНИЙ ЭТОГО КИТА"
                }
                , this, "ru");
            ...
            lang.RegisterMessages(new Dictionary<string, string>
                {
                    ...
                    ["UI_Minutes"]            = "{0}m",
                    ["UI_Seconds"]            = "{0}s",
                    ["KitUsageLimitReached"]  = "YOU HAVE REACHED THE USAGE LIMIT FOR THIS WHALE"
                }
                , this);
        }
        ...
        public void ViewKitList(BasePlayer player)
        {
            ...
            float position   = 0.506f - list.Skip((String - 1) * 4).Take(4).Count() / 2f * width - (list.Skip((String - 1) * 4).Take(4).Count()) / 2f * margin;
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                var listElement = element.KitList.Where(p => {
                    if (p.Hide || p.DisplayName.Equals("autokit", StringComparison.OrdinalIgnoreCase)) 
                        return false;
                        
                    // Проверяем лимит использований
                    if (p.UsageLimit > 0)
                    {
                        var playerCooldown = _playerData[player.userID]
                            .FirstOrDefault(cd => cd.DisplayName.Equals(p.Name, StringComparison.OrdinalIgnoreCase) && cd.Number == element.Number);
                            
                        if (playerCooldown != null && playerCooldown.UsageCount >= p.UsageLimit)
                            return false;
                    }
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    var hasPermission = permission.UserHasPermission(player.UserIDString, elementKitList.Permission);

                    var findCooldown = _playerData[player.userID]
                        .FirstOrDefault(p => p.DisplayName.Equals(elementKitList.Name, StringComparison.OrdinalIgnoreCase) && p.Number == element.Number);
                    ...
                    if (findCooldown != null)
                    {
                        ...
                        firstContainer.Add(new CuiLabel
                        {
                            Text          = {Text      = elementKitList.DisplayName, Font = "robotocondensed-regular.ttf", FontSize = 13, Align = TextAnchor.MiddleLeft, Color = HexToCuiColor("#E4DAD1")},
                            RectTransform = {AnchorMin = $"0.03333616 0", AnchorMax = $"0.3 1" },
                        }, Layer + $"{j}.U");
                    ...
                    else
                    {
                        if (!hasPermission)
                        {
                            ...
                            firstContainer.Add(new CuiLabel
                            {
                                Text          = {Text = elementKitList.NoPermissionMessage, Font = "robotocondensed-regular.ttf", FontSize = 10, Align = TextAnchor.MiddleRight, Color = HexToCuiColor("#E4DAD1", 100)},
                                RectTransform = {AnchorMin = $"0.25 0", AnchorMax = $"0.8045457 1" },
                            }, Layer + $"{j}.U");
                        }
                        else
                        {
                    ...
                    if (findCooldown == null && hasPermission)
                    {
